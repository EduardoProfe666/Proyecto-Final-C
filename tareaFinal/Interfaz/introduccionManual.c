...
void agregarDeporte(BD *baseDatos,COORD posicionMarco){
    ...
    do{
        ...
        if(deporteValidado)
            deporteValidado=validarRepeticionDeporte(*baseDatos,respTemp);
        if(deporteValidado){
            sprintf(baseDatos->baseDeportes[baseDatos->cantidadDeportesAlmacenados],respTemp);
...
void modificarDeporte(BD *baseDatos,COORD posicionMarco){
    ...
    do{
        ...
        modificarColor(VERDE);
        printf("%s",baseDatos->baseDeportes[posDeporte]);
        modificarColor(BLANCO);
        printf(".");
        ...
        if(deporteValidado)
            deporteValidado=validarRepeticionDeporte(*baseDatos,respTemp);
        if(deporteValidado)
            sprintf(baseDatos->baseDeportes[posDeporte],respTemp);
        else{
            imprimirMarcoGeneral(2,1,posicionMarco,LONG_ALTO_MARCO);
...
void mostrarDeportes(BD baseDatos,COORD posicionMarco){
    int i;

    imprimirMarcoGeneral(4,1,posicionMarco,baseDatos.cantidadDeportesAlmacenados+8);
    ...
    for(i=0;i<baseDatos.cantidadDeportesAlmacenados;i++){
        moverCursor(posicionMarco.X+4,5+i+posicionMarco.Y);
        printf("%-2d-. %s",i+1, baseDatos.baseDeportes[i]);
    }

    moverCursor(posicionMarco.X+5,posicionMarco.Y+baseDatos.cantidadDeportesAlmacenados+7);
    modificarColor(REINICIAR_COLOR);
    system("pause");
}

void modificarDeportes(BD *baseDatos,COORD posicionMarco){
    int eleccion=-1;
    ...
    do{
        ...
        if(CANT_MAX_DEPORTES-baseDatos->cantidadDeportesAlmacenados==0)
            modificarColor(ROJO);
        else
            modificarColor(VERDE);
        printf("%d",CANT_MAX_DEPORTES-baseDatos->cantidadDeportesAlmacenados);
        modificarColor(BLANCO);
        ...
        case 0:
            ...
            break;
        case 1:
            mostrarDeportes(*baseDatos,posicionMarco);
            break;
        case 2:
            modificarDeporte(baseDatos,posicionMarco);
            comunicarOperacionExito(posicionMarco);
            break;
        case 3:
            if(baseDatos->cantidadDeportesAlmacenados==1)
                comunicarMinimoDeportesAlcanzadoError(posicionMarco);
            else{
                eliminarDeporte(baseDatos,posicionMarco);
                comunicarOperacionExito(posicionMarco);
            }
            break;
        case 4:
            if(baseDatos->cantidadDeportesAlmacenados==CANT_MAX_DEPORTES)
                comunicarMaximoDeportesAlcanzadoError(posicionMarco);
            else{
                agregarDeporte(baseDatos,posicionMarco);
                comunicarOperacionExito(posicionMarco);
            }
            break;
        default:
...
bool pedirNombreYSexo(char nombre[],char *sexo,int anyo,int numeroParticipante,int contadorComandoAleatorio,BD baseDatos,COORD posicionMarco){
    ...
    do{
        ...
        if(comandoAleatorizar){
            ...
            *sexo=aleatorizarNombre(baseDatos,nombre);
        }
        else{
...
void pedirDeporte(char deporte[],BD baseDatos,int anyo,int numeroParticipante,COORD posicionMarco){
    ...
    do{
        imprimirMarcoGeneral(1,1,posicionMarco,baseDatos.cantidadDeportesAlmacenados+14);
        moverCursor((LONG_ANCHO_MARCO-17)/2+1+posicionMarco.X,posicionMarco.Y+1);
        ...
        moverCursor(posicionMarco.X+5,posicionMarco.Y+13+baseDatos.cantidadDeportesAlmacenados);
        modificarColor(REINICIAR_COLOR);
        fflush(stdin);
        fgets(respTemp,sizeof(respTemp),stdin);
        ...
        if(comandoAleatorizar){
            entradaValidada=true;
            aleatorizarDeporte(baseDatos,deporte);
        }
        else{
            entradaValidada=validarNumeroCorrecto(respTemp,baseDatos.cantidadDeportesAlmacenados,1);
            if(entradaValidada){
                seleccion=atoi(respTemp);
                sprintf(deporte,baseDatos.baseDeportes[seleccion-1]);
            }
            else{
                imprimirMarcoGeneral(2,1,posicionMarco,LONG_ALTO_MARCO);
                moverCursor(posicionMarco.X+4,posicionMarco.Y+4);
                printf("Debe introducir  un n\243mero entero  entre 1 y %d. En  dependencia de su  selecci\242n, el programa",baseDatos.cantidadDeportesAlmacenados);
                moverCursor(posicionMarco.X+4,posicionMarco.Y+5);
                printf("ejecutar\240  la acci\242n  que  le corresponde  el  n\243mero  introducido. Tome  como gu\241a  la lista");
                moverCursor(posicionMarco.X+4,posicionMarco.Y+6);
...
bool pedirDatosRestantes(DatosAnyo *datosAnyo,BD baseDatos,COORD posicionMarco,int contadorComandoAleatorioGlobal){
    ...
    do{
        ...
        if(comandoAleatorizarGlobal){
            ...
            rellenarDatosPersonales(baseDatos,datosAnyo,0);
        }
        else{
                ...
                if(cantidadParticipantes!=0){
                    ...
                    for(posicion=0;posicion<cantidadParticipantes;posicion++){
                        if(comandoAleatorizar==3){
                            rellenarDatosPersonales(baseDatos,datosAnyo,posicion);
                        }
                        else{
                            comandoAleatorio=false;
                            comandoAleatorio=pedirNombreYSexo(datosAnyo->participantesAnyo[posicion].nombre,&datosAnyo->participantesAnyo[posicion].sexo,datosAnyo->numeroAnyo,posicion+1,comandoAleatorizar,baseDatos,posicionMarco);

                            if(comandoAleatorio){
                                aleatorizarDeporte(baseDatos,datosAnyo->participantesAnyo[posicion].deporte);
                                comandoAleatorizar++;
                            }
                            else{
                                pedirDeporte(datosAnyo->participantesAnyo[posicion].deporte,baseDatos,datosAnyo->numeroAnyo,posicion+1,posicionMarco);
...
bool pedirNombreFeu(DatosAnyo *datosAnyo,BD baseDatos,COORD posicionMarco,int contadorComandoAleatorio){
    bool comandoAleatorizar=false;
    bool entradaValidada=true;
    char respTemp[MAX_LONG_CADENA_CARACTERES+1];
    ...
    do{
        ...
        if(comandoAleatorizar){
            entradaValidada=true;
            aleatorizarNombre(baseDatos,datosAnyo->nombreResponsable);
        }
        else{
            entradaValidada=validarNombre(respTemp);
            if(entradaValidada)
...
void introducirDesdeCero(DatosAnyo datosAnyo[],BD baseDatos,int *cantidadAnyos,COORD posicionMarco){
    ...
    for(i=0;i<*cantidadAnyos;i++){
        if(comandoAleatorizar==3)
            aleatorizarNombre(baseDatos,datosTemp[i].nombreResponsable);
        else{
            comandoAleatorio=false;
            comandoAleatorio=pedirNombreFeu(&datosTemp[i],baseDatos,posicionMarco,comandoAleatorizar);
    ...
    for(i=0;i<*cantidadAnyos;i++){
        if(comandoAleatorizar==3){
            ...
            rellenarDatosPersonales(baseDatos,&datosTemp[i],0);
        }
        else{
            comandoAleatorio=false;
            comandoAleatorio=pedirDatosRestantes(&datosTemp[i],baseDatos,posicionMarco,comandoAleatorizar);
...
void modificarDatos(DatosAnyo datosAnyo[],BD baseDatos,int cantidadAnyos,COORD posicionMarco){
    ...
    do{
        ...
        case 1:
            ...
            break;
        case 2:
            pedirNombreFeu(&datosAnyo[preguntarAnyo(datosAnyo,cantidadAnyos,posicionMarco)],baseDatos,posicionMarco,-1);
            comunicarOperacionExito(posicionMarco);
            break;
        case 3:
            pedirDatosRestantes(&datosAnyo[preguntarAnyo(datosAnyo,cantidadAnyos,posicionMarco)],baseDatos,posicionMarco,-1);
            comunicarOperacionExito(posicionMarco);
            break;
        case 4:
        ...
        case 5:
            ...
            if(datosAnyo[temp1].cantParticipantes==0)
                ...
                pedirNombreYSexo(datosAnyo[temp1].participantesAnyo[temp2].nombre,&datosAnyo[temp1].participantesAnyo[temp2].sexo,datosAnyo[temp1].numeroAnyo,temp2+1,-1,baseDatos,posicionMarco);
                comunicarOperacionExito(posicionMarco);
            }
            break;
        case 6:
            ...
            if(datosAnyo[temp1].cantParticipantes==0)
                ...
                temp2=preguntarParticipante(datosAnyo[temp1],posicionMarco);
                pedirDeporte(datosAnyo[temp1].participantesAnyo[temp2].deporte,baseDatos,datosAnyo[temp1].numeroAnyo,temp2+1,posicionMarco);
                comunicarOperacionExito(posicionMarco);
            }
            break;
        default:
...
void introducirDatosUsuario(DatosAnyo datosAnyo[], BD *baseDatos, int *cantidadAnyos, bool *datosExistentes, COORD posicionMarco){
    int eleccion;
    char eleccionTemp[TAM_CHAR_VAR_VALIDAR];
    bool salida=false;
    bool respTemp=true;
    ...
    do{
        ...
        case 0:
            ...
            break;
        case 1:
            introducirDesdeCero(datosAnyo,*baseDatos,cantidadAnyos,posicionMarco);
        ...
        case 2:
            if(!*datosExistentes)
                ...
                modificarDatos(datosAnyo,*baseDatos,*cantidadAnyos,posicionMarco);
            break;
        case 3:
            if(*datosExistentes)
                respTemp=preguntarDatosExistentesAdv(posicionMarco,2);
            if(respTemp){
                modificarDeportes(baseDatos,posicionMarco);
