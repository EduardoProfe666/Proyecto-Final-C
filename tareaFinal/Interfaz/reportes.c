...
void mostrarReporteParticipantesDadoAnyoDeporte(DatosAnyo datosAnyo[],BD baseDeDatos,int cantidadAnyos,COORD posicionMarco){
    int i, k;
    ...
    for(i=0,k=0;i<datosAnyo[anyoSeleccionadoPos].cantParticipantes;i++,k++){
        ...
        if(strcmp(datosAnyo[anyoSeleccionadoPos].participantesAnyo[i].deporte, baseDeDatos.baseDeportes[deporteSeleccionadoPos])==0){
    ...
    if(cantidadParticipantesSeleccionados==0){ //Caso Extremo
        ...
        printf("el deporte");
        modificarColor(VERDE);
        printf(" %s", baseDeDatos.baseDeportes[deporteSeleccionadoPos]);
        modificarColor(BLANCO);
        printf(".");
    }
    else{
