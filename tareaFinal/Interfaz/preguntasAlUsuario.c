...
int preguntarDeporte(BD baseDatos,COORD posicionMarco){
    int deportePos;
    ...
    do{
        ...
        imprimirMarcoGeneral(1,1,posicionMarco,baseDatos.cantidadDeportesAlmacenados+8);
        ...
        moverCursor(posicionMarco.X+5,baseDatos.cantidadDeportesAlmacenados+7+posicionMarco.Y);
        modificarColor(REINICIAR_COLOR);

        fflush(stdin);
        fgets(deportePosChar,sizeof(deportePosChar),stdin);

        numeroValidado=validarNumeroCorrecto(deportePosChar,baseDatos.cantidadDeportesAlmacenados,1);
        if(numeroValidado)
