...
bool validarRepeticionDeporte(BD baseDatos,char cadenaIntroducida[]){
    ...
    for(i=0;i<baseDatos.cantidadDeportesAlmacenados && noRepetido;i++){
        char DeporteTemp[MAX_LONG_CADENA_CARACTERES];
        strcpy(DeporteTemp,baseDatos.baseDeportes[i]);
        strlwr(DeporteTemp);
